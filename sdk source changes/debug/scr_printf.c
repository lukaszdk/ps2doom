...
#
# $Id: scr_printf.c 783 2005-01-22 12:04:10Z drakonite $
...
_putchar( int x, int y, u32 color, u8 ch)
{
   ...
   for (i=l=0; i < 8; i++, l+= 8, font++)
      for (j=0; j < 8; j++)
          ...
          if ((*font & (128 >> j)))
          {
              pixel = color;      // original
              //if(i/2*2 == i)
              //  pixel = color;
              //else
              //{
              //    // obter cor metade to brilho
              //    if(color == 0xffffff)
              //      pixel = 0xff5f5f;
              //    else
              //      pixel = 0;
...
void scr_printf_nocursor(const char *format, ...)
{
   ...
   for (i = 0; i < bufsz; i++)
    ...
    // _putchar( X*7 , Y * 8, 0xffffff, 219);       // cosmito : no cursor
}
